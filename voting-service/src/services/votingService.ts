...
export const votingService = {
	ApplyUpVote: async (call: ServerUnaryCall<any, any>, callback: sendUnaryData<any>) => {
	...
	ApplyDownVote: async (call: ServerUnaryCall<any, any>, callback: sendUnaryData<any>) => {
		const userId = await getAuthenticatedUserId(call.metadata);
		if (!userId) {
			return callback({
				code: status.UNAUTHENTICATED,
	...
	GetCountVote: async (call: ServerUnaryCall<any, any>, callback: sendUnaryData<any>) => {
		const { isThread, targetId } = call.request;
		const result = await getCountVote(isThread, targetId);
		callback(null, result);
	},
	IsUserVote: async (call: ServerUnaryCall<any, any>, callback: sendUnaryData<any>) => {
		const userId = await getAuthenticatedUserId(call.metadata);
		if (!userId) {
			return callback({
				code: status.UNAUTHENTICATED,
