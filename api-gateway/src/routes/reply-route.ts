...
replyRouter.post("/:threadId", async (req: Request, res: Response) => {
	...
	try {
		const response = await axios.post(`${REPLY_SERVICE_URL}/replies/${threadId}`, req.body, {
			headers: {
...
replyRouter.get("/:threadId", async (req: Request, res: Response) => {
	const { threadId } = req.params;
	try {
		const response = await axios.get(`${REPLY_SERVICE_URL}/replies/${threadId}`, {
			headers: {
				Authorization: req.headers.authorization, // Add the token to the request headers
			},
		});
...
replyRouter.get("/:threadId/:replyId", async (req: Request, res: Response) => {
	...
	try {
		const response = await axios.get(`${REPLY_SERVICE_URL}/replies/${threadId}/${replyId}`, {
			headers: {
				Authorization: req.headers.authorization, // Add the token to the request headers
			},
		});
...
replyRouter.delete("/:threadId/:replyId", async (req: Request, res: Response) => {
	const { threadId, replyId } = req.params;
	try {
		const response = await axios.delete(`${REPLY_SERVICE_URL}/replies/${threadId}/${replyId}`, {
			headers: {
				Authorization: req.headers.authorization, // Add the token to the request headers
			},
		});
