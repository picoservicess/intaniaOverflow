 ...
 */
router.put("/:threadId/:replyId", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    if (existingReply.userId !== userId) {
      ...
      res.status(403).json({ error: "You are not authorized to edit this reply" });
      return
    }
 ...
 * Access: Requires authentication and ownership of the reply
 */
router.delete("/:threadId/:replyId", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    // Extract replyId from the URL parameters
    const { threadId, replyId } = req.params;
    // Get userId from authenticated request
