        ...
        async (msg) => {
            ...
            try {
                ...
                if (msg.fields.routingKey == "reply") {
                    console.log(
                        "✉️ Received message for reply:",
                        message.replyId
                    );
                    try {
                        const sendingPromise = userIds.map(async (userId) => {
                            const notiMessage = await parseReply(
                                message,
                                userId
                            );
                        ...
                        console.log(
                            "Fail to create notification from reply: ",
                            error
                        );
                    }
                }
                if (msg.fields.routingKey == "thread") {
                    console.log(
                        "✉️ Received message for thread:",
                        message.threadId
                    );
                    try {
                        const sendingPromise = userIds.map(async (userId) => {
                            const notiMessage = await parseThread(
                                message,
                                userId
                            );
                            await createNotification(notiMessage);
                        ...
                        console.log(
                            "Fail to create notification from thread: ",
                            error
                        );
                    }
                }
