			...
			try {
				...
				if (token) {
					...
					threads.forEach((thread) => {
						if (
							myThreads.threads.some((myThread: Thread) => myThread.threadId === thread.threadId)
						) {
				...
				<div className="space-y-4">
					{loading ? (
						...
						<div className="w-full flex justify-center items-center p-8">
							<p className="text-gray-500 text-center text-sm sm:text-base">ไม่พบผลลัพธ์ที่ค้นหา</p>
						</div>
					)}
				</div>
				{paginationLoading ? (
