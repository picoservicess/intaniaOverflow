...
const VoteSection: React.FC<VoteSectionProps> = ({ voteCount, voteStatus, isThread, targetId }) => {
	const { data: session } = useSession();
	const [vote, setVote] = useState<VoteCounts>(voteCount);
	const [status, setStatus] = useState<number>(voteStatus);

	const handleUpVote: React.MouseEventHandler<HTMLButtonElement> = async (event) => {
	...
	const handleDownVote: React.MouseEventHandler<HTMLButtonElement> = async (event) => {
		event.stopPropagation();
		if (!session) return;
		const token = session.user.accessToken as string;
